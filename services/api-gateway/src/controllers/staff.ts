...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  const { name, email, phone, role, department, dob, gender, salary, employee_no, photo_url } = req.body;
  if (!name) { res.status(400).json({ error: 'name is required' }); return; }
  const [row] = await sql`
    INSERT INTO staff (tenant_id, name, email, phone, role, department, dob, gender, salary, employee_no, photo_url)
    VALUES (${req.tenantId!}, ${name}, ${email||null}, ${phone||null}, ${role||'instructor'},
            ${department||null}, ${dob||null}, ${gender||null}, ${salary||null},
            ${employee_no||null}, ${photo_url||null})
    RETURNING *
  `;
...
router.get('/:id', async (req: Request, res: Response): Promise<void> => {
  const [row] = await sql`SELECT * FROM staff WHERE id = ${req.params.id} AND tenant_id = ${req.tenantId!}`;
