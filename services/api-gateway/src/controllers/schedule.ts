...
router.post('/slots', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!start_time || !end_time) { res.status(400).json({ error: 'start_time and end_time required' }); return; }
  const [row] = await sql`
    INSERT INTO schedule_slots (tenant_id, batch_id, staff_id, room, day_of_week, slot_date, start_time, end_time)
    VALUES (${req.tenantId!}, ${batch_id||null}, ${staff_id||null}, ${room||null},
...
router.delete('/slots/:id', async (req: Request, res: Response): Promise<void> => {
  await sql`DELETE FROM schedule_slots WHERE id = ${req.params.id} AND tenant_id = ${req.tenantId!}`;
  res.status(204).send();
});

// GET /erp/schedule/conflicts — find double-booked staff or rooms
router.get('/conflicts', async (req: Request, res: Response): Promise<void> => {
  const rows = await sql`
    SELECT a.id as slot_a, b.id as slot_b, a.staff_id, a.room, a.day_of_week, a.start_time, b.start_time as b_start
    FROM   schedule_slots a
    JOIN   schedule_slots b ON a.tenant_id = b.tenant_id
      AND  a.id < b.id
      AND  a.day_of_week = b.day_of_week
      AND  a.is_cancelled = FALSE AND b.is_cancelled = FALSE
