...
router.get('/', async (req: Request, res: Response): Promise<void> => {
  ...
  const rows = await sql`
    ...
    WHERE  l.tenant_id = ${req.tenantId!}
      ...
      AND  (${search||null}::text IS NULL OR l.name ILIKE ${'%'+(search||'')+'%'} OR l.phone ILIKE ${'%'+(search||'')+'%'})
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  const [row] = await sql`
    ...
    VALUES (${req.tenantId!}, ${name}, ${email||null}, ${phone||null}, ${source||null},
            ${course_id||null}, ${notes||null}, ${assigned_to||null})
    RETURNING *
  `;
...
// POST /erp/admissions/:id/convert  — convert lead to member
router.post('/:id/convert', async (req: Request, res: Response): Promise<void> => {
  const [lead] = await sql`SELECT * FROM leads WHERE id = ${req.params.id} AND tenant_id = ${req.tenantId!}`;
  ...
  try {
    ...
    `;
    await sql`
      UPDATE leads SET stage = 'converted', converted_at = NOW(), member_id = ${member.id}, updated_at = NOW()
