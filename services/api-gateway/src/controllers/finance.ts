...
// GET /erp/finance/fee-plans
router.get('/fee-plans', async (req: Request, res: Response): Promise<void> => {
  const rows = await sql`
    SELECT * FROM fee_plans WHERE tenant_id = ${req.tenantId!} ORDER BY name
  `;
  res.json(rows);
});

// POST /erp/finance/fee-plans
...
// PATCH /erp/finance/fee-plans/:id
router.patch('/fee-plans/:id', async (req: Request, res: Response): Promise<void> => {
  ...
  const [row] = await sql`
    ...
    RETURNING *
  `;
  if (!row) { res.status(404).json({ error: 'Fee plan not found' }); return; }
  res.json(row);
});

// DELETE /erp/finance/fee-plans/:id
...
router.post('/invoices', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!member_id || !amount) {
    res.status(400).json({ error: 'member_id and amount required' }); return;
  }
...
// POST /erp/finance/invoices/:id/send
  ...
  try {
    const [pay] = await sql`
      INSERT INTO payments
        (tenant_id, member_id, invoice_id, amount, method, reference, paid_on, notes)
      VALUES
        (${req.tenantId!}, ${member_id}, ${invoice_id || null}, ${amount},
         ${method || 'cash'}, ${reference || null},
         ${paid_on || new Date().toISOString().slice(0, 10)}, ${notes || null})
      RETURNING *
    `;
...
// POST /erp/finance/payments/:id/refund
...
// GET /erp/finance/reports/revenue
...
// GET /erp/finance/reports/outstanding
