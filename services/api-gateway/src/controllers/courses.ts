...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  const [row] = await sql`
    ...
    VALUES (${req.tenantId!}, ${name}, ${code||null}, ${description||null}, ${category||null},
            ${duration||null}, ${capacity||null}, ${fee||0})
    RETURNING *
  `;
...
router.get('/:id', async (req: Request, res: Response): Promise<void> => {
  const [row] = await sql`SELECT * FROM courses WHERE id = ${req.params.id} AND tenant_id = ${req.tenantId!}`;
...
router.post('/:id/batches', async (req: Request, res: Response): Promise<void> => {
  ...
  const [row] = await sql`
    ...
    VALUES (${req.tenantId!}, ${req.params.id}, ${name}, ${start_date||null}, ${end_date||null}, ${capacity||null})
    RETURNING *
  `;
  res.status(201).json(row);
});
...
router.get('/:id/members', async (req: Request, res: Response): Promise<void> => {
  const rows = await sql`
    SELECT m.id, m.name, m.email, m.phone, e.enrolled_on, e.status as enrollment_status, b.name as batch_name
    FROM   enrollments e
